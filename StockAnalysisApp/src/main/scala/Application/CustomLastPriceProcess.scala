  ...
  override def processElement(i: EventUnpackSchema,
    ...
    if (i.Price == -1.0) {
      out.collect(LastPriceOutputSchema(Symbol = i.Symbol,
                                        ...
                                        lookupSymbolBool = i.lookupSymbolBool,
                                        lookupSize = i.lookupSize,
                                        WindowLastTS = i.Timestamp,
                                        WindowClosingPrice = -1.0,
                                        isLastBatch = i.isLastBatch,
                                        isLastWindow = true))
    } else {
      ...
      if (!batchState.contains(i.batchID)) {
        batchState.put(i.batchID, BatchInternalSchema(isClosed = false,
                                                      hasSent = false,
                                                      isLookupBool = i.lookupSymbolBool,
                                                      lookupSize = i.lookupSize,
                                                      isLastBatch = i.isLastBatch))
      }
      ...
      if (windowState.get(windowGroupRunningTS).last_timestamp < i.Timestamp || ((windowState.get(windowGroupRunningTS).last_timestamp == i.Timestamp && windowState.get(windowGroupRunningTS).last_price < i.Price))) {
      ...
      if (i.LastInBiBool) {
        ...
        completedWindowList.foreach(elem => {
          ...
          out.collect(LastPriceOutputSchema(Symbol = i.Symbol,
                                            securityType = i.securityType,
                                            batchID = i.batchID,
                                            lookupSymbolBool = curBatchState.isLookupBool,
                                            lookupSize = curBatchState.lookupSize,
                                            WindowLastTS = tempWindowState.last_timestamp,
                                            WindowClosingPrice = tempWindowState.last_price,
        ...
        if (!curBatchState.hasSent && curBatchState.isLookupBool) {
          ...
          out.collect(LastPriceOutputSchema(Symbol = i.Symbol,
                                            ...
                                            lookupSymbolBool = curBatchState.isLookupBool,
                                            lookupSize = curBatchState.lookupSize,
                                            WindowLastTS = uncompletedWindow.last_timestamp,
                                            WindowClosingPrice = -1.0,
                                            isLastBatch = curBatchState.isLastBatch,
                                            isLastWindow = true))
