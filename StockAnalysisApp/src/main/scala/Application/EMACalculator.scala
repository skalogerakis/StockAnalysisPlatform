...
class EMACalculator(j: Double = 38.0,
                    w: Double = 100.0) extends RichMapFunction[LastPriceOutputSchema, EMASchema] {
  ...
  override def map(in: LastPriceOutputSchema): EMASchema = {
    ...
    if (in.WindowClosingPrice == -1.0) {
      EMASchema(Symbol = in.Symbol,
                security_type = in.securityType,
                batchID = in.batchID,
                lookupSymbolBool = in.lookupSymbolBool,
                lookupSize = in.lookupSize,
                WindowLastTS = in.WindowLastTS,
                WindowClosingPrice = in.WindowClosingPrice,
                isLastBatch = in.isLastBatch,
                EMA_j = lastEMAj.value(),
                EMA_w = lastEMAw.value(),
